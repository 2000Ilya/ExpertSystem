    ...
    if (localStorage.getItem("isAuth")) {
      return (
        <div className="expert">
          ...
          <input
            ...
            value={newGame.pictureLink}
            onChange={(e) => {
              setNewGame({ ...newGame, ["pictureLink"]: e.target.value });
            }}
            className="text"
          />
          <input
            type="text"
            placeholder="Добавьте рейтинг игры"
            value={newGame.rating}
            onChange={(e) => {
              setNewGame({ ...newGame, ["rating"]: Number(e.target.value) });
            }}
            className="text"
          />
          ...
          {questionsData.map((el, index) => {
            return (
              <div key={index} className="selectType">
                ...
                <select
                  value={
                    newGame[el.type] ? newGame[el.type] : el.answers[0].text
                  }
                  onChange={(e) => {
          ...
          <div className="buttons">
            <button onClick={checkData} className="add">
              Добавить
            </button>
      ...
      return (
        <div className="reject">
          <h1 className="alert">
            Авторизируйтесь как эсперт, чтобы иметь возможность добавлять игры
          </h1>
          <button onClick={() => quit()} className="quit">
            Выйти
          </button>
